...
class SynergyNet(keras.Model):
    ...
    def call(self, input):
        ...
        S1S2_loss = tf.reduce_mean(tf.math.square(_3D_attr[:,:12] - _3D_attr_S2[:,:12]), axis=-1) \
                  + tf.reduce_mean(tf.math.square(_3D_attr[:,12:] - _3D_attr_S2[:,12:]), axis=-1)

        
        self.add_loss(0.001*S1S2_loss)

        return {"pred_param":_3D_attr, 
                "pred_lmk": vertex_lmk,
                "refined_lmk": vertex_lmk_refine,
                "refined_param": _3D_attr_S2
                }
...
class I2P(tf.keras.layers.Layer):
...
class MLP_for(keras.layers.Layer):
...
class MLP_rev(keras.layers.Layer):
    def __init__(self, num_pts):
        super().__init__()
